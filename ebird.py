...
def get_recent_obs(regionCode='CN-11',back=7):
...
def get_historic_obs(regionCode='CN-11', date=''):
...
def get_historic_list(regionCode='CN-11', date=''):
...
def get_report_detail(token, subId):
...
def search(taxonid='', startTime='', endTime='', province='', city='', district='', pointname='', username='', serial_id='', ctime='', taxonname='', state='', token=''):
    id_list = []
    id_detail = {}

    ta_s = date_to_ta(startTime)
    ta_e = date_to_ta(endTime)
    # print(ta_s, ta_e)
    back = int((ta_e - ta_s) / 86400)
    # print(back)
    date_list = [ta_to_date(ta_s+86400*i) for i in range(back+1)]
    print('查询日期\n',date_list)

    print('正在获取日期范围内的checklist id...')

    for _date in date_list:
        res = get_historic_list(date=_date)
        # print(json.dumps(res,sort_keys=True, indent=4, separators=(',', ': ')))
    ...
    for _id in id_list:
        detail = get_report_detail(token=token, subId=_id)
...
startTime = get_back_date(7)
search(startTime=startTime,endTime=endTime,token=token)
